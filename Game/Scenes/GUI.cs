...
namespace OpenGL_Game.Game.Scenes
{
    static class GUI
    {
        ...
        public static void SetUpGUI(int width, int height)
        {
            ...
            geometry = ResourceManager.LoadGeometry("Game/Geometry/Text/text.obj");

            pgmID = GL.CreateProgram();
            int vsID;
            int fsID;
            LoadShader("Engine/Shaders/text.vert", ShaderType.VertexShader, pgmID, out vsID);
            LoadShader("Engine/Shaders/text.frag", ShaderType.FragmentShader, pgmID, out fsID);
